module "ecs_app_my_app" {
  source = "git::https://github.com/EluminaGroup/terraform-aws-ecs-app.git?ref=1.0.0" # check for latest version

  name           = "my-app"
  image          = "learnawstechclub/nginx-hello:latest"
  container_port = 80
  cpu            = 512
  memory         = 2048
  ...
  service_role_arn                 = module.ecs_apps.ecs_service_iam_role_arn                  # from https://github.com/EluminaGroup/terraform-aws-ecs
  task_role_arn                    = module.ecs_apps.ecs_task_iam_role_arn                     # from https://github.com/EluminaGroup/terraform-aws-ecs
  alb_listener_https_arn           = element(module.ecs_apps.alb_listener_https_arn, 0)        # from https://github.com/EluminaGroup/terraform-aws-ecs
  test_traffic_route_listener_arn  = element(module.ecs_apps.alb_listener_test_traffic_arn, 0) # from https://github.com/EluminaGroup/terraform-aws-ecs
  codedeploy_wait_time_for_cutover = 1440

  hostnames          = ["my-app.dev.cloud.elumina.com.au"]
  hostname_redirects = ["www.my-app.dev.cloud.elumina.com.au"]
  hostname_create    = true
  alb_only           = true                                     # cluster also need this option to disable ALB WAF
  alb_dns_name       = element(module.ecs_apps.alb_dns_name, 0) # from # https://github.com/EluminaGroup/terraform-aws-ecs
  hosted_zone        = "dev.cloud.elumina.com.au"
  paths              = ["/*"]
  ...
  fargate_spot    = true
  network_mode    = "awsvpc"
  security_groups = [module.ecs_apps.ecs_nodes_secgrp_id] #  # from # https://github.com/EluminaGroup/terraform-aws-ecs
...
module "ecs_apps" {
  source = "git::https://github.com/EluminaGroup/terraform-aws-ecs.git?ref=1.0.0" # check for latest version
